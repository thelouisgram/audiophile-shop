  ...
  return (
    ...
    >
      ...
      <div className="md:w-1/2">
        { }
        <img src={item.categoryImage.desktop} className="rounded-[10px] hidden md:flex" />
        {/* Tablet image */}
        <img src={item.categoryImage.tablet} className="rounded-[10px] hidden ss:flex md:hidden" />
        {/* Mobile image */}
        <img src={item.categoryImage.mobile} className="rounded-[10px] flex ss:hidden" />
      </div>
      <div className={`md:w-1/2 w-full text-center md:text-left ${index % 2 === 0 ? "md:pl-24" : "pl-0"}`}>
        ...
        {/* Description */}
        <p className='text-[13px] xs:text-[15px] leading-[1.5em] text-elements ss:w-[625px] ss:mx-auto md:mx-0 md:w-[450px] mb-8'>{item.description}</p>
        {/* See product button */}
